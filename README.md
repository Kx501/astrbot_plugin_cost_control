...
>
> **工具循环的统计边界**：AstrBot 4.25.5 的原生统计按一次 agent 执行记录累计 Token，响应钩子只提供最后一轮响应。本插件通过兼容路径补齐同一请求的累计 Token，并标记原始缓存字段仅覆盖最终响应。`per_turn` 次数、逐轮缓存创建量、阶梯/表达式价格无法从这些数据恢复为逐次模型调用的精确账单；未暴露累计统计的宿主版本会回退最终响应采样。
